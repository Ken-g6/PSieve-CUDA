...
cd ..
zip -9 $apppath/$appname-src.zip Makefile *.txt *.[ch] *.sln $apppath/*.[ch] $apppath/*.cu $apppath/README.txt $apppath/CHANGES.txt $apppath/*config.txt $apppath/make-*.sh $apppath/$appname.sh $apppath/*.bat vc/*
