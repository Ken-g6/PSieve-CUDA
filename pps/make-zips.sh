...
cd ..
zip -9 $apppath/$appname-src.zip Makefile *.txt *.[ch] $apppath/*.[ch] $apppath/*.cu $apppath/README.txt $apppath/CHANGES.txt $apppath/*config.txt $apppath/make-*.*[ht] $apppath/$appname.sh $apppath/$appname.bat vc/*
